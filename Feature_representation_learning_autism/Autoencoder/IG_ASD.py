...
#file=pd.read_excel('subjects.xlsx')
...
from torch.optim import Adam, lr_scheduler
    ...
    #ASD / Control data split (asd)
file_asd= file[file['Gk_II']=='ASD']
file_con= file[file['Gk_II']=='CONTROL']
ASD_data = file_asd['data'].values
a=file['data'].values
x_data=np.zeros((47,9900))
for i in range(0,47):
    for j in range(0,9900):
        x_data[i][j]=ASD_data[i][j]
...
x_test_matrix = np.zeros((47, 200, 200))
matrix_sub = np.zeros((47, 200, 200))
for tnum in range(0,47):
