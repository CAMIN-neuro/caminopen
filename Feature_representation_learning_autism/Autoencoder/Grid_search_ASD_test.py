...
for what in ['dropout','lr','layer','unit']:
    ...
    for grid in [1,2]:
        ...
        model = AutoEncoder(layer=nlayer, unit_=unit, dropout=dropout).to(device='cuda:1')


        #lr = 0.0001
        device = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')

        main_path = '/camin1/yrjang/Autism classification/Auto_revision/SDSU/grid_sdsu/asd/model_d{}_u{}_layer{}_lr{}'.format(dropout,unit[0],nlayer, lr)
        ...
        # save result
        file = open('/camin1/yrjang/Autism classification/Auto_revision/SDSU/asd_model_test_text/model_d{}_u{}_layer{}_lr{}_asd_test.txt'.format(dropout, unit[0],
                                                                                                  nlayer, lr), 'w')
...
icc_t = []
for i in range(0,4):
    dat = pd.concat([pd.DataFrame(x_test_n[i]), pd.DataFrame(x_hat_test_n[i])], axis=1)
    ns = len(dat)  # 행 개수
    nr = len(dat.columns)  # 열 개수
